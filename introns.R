...
hist = ggplot() + 
  geom_histogram(aes(sims$purine_content), binwidth = 0.01, fill = "RoyalBlue", col = "black") +
  geom_vline(xintercept = real$purine_content, lty = 1, size = 1.5, col = "red") +
  labs(x = "Motif set purine content", y = "Frequency")
...
plot = ggplot(data=sims, aes(g_content,a_content)) + 
  geom_point(cex = 0.8) +
  # stat_density2d(aes(fill=..level..,alpha=..level..),geom='polygon',colour='#cccccc') +
  geom_point(aes(x = real$g_content, y = real$a_content), col = "red", pch = 18, cex = 3) +
