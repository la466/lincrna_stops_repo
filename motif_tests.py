...
def main():
    ...
    arguments = ["output_directory", "motif_file", "required_simulations", "motif_codon_densities"]

    description = ""
    args = gen.parse_arguments(description, arguments, flags = [3], ints=[2])
    output_directory, motif_file, required_simulations, motif_codon_densities = args.output_directory, args.motif_file, args.required_simulations, args.motif_codon_densities


    motif_controls_directory = "{0}/{1}_dinucleotide_controls".format(output_directory, motif_file.split("/")[-1].split(".")[0])
